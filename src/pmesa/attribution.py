...
def path_integrated_gradients(score: ScalarScore, paths: Sequence[RestorationPath]) -> PathAttribution:
    ...
    with torch.enable_grad():
        baseline_score = score(paths[0].states[0].to(dtype=torch.get_default_dtype()))
        input_score = score(paths[0].states[-1].to(dtype=torch.get_default_dtype()))
        score_delta = (input_score - baseline_score).detach()
        for path in paths:
            states = path.states.to(dtype=torch.get_default_dtype())
